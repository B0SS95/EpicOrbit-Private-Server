using EpicOrbit.Emulator.Netty.Attributes;
using EpicOrbit.Emulator.Netty.Interfaces;
namespace EpicOrbit.Emulator.Netty.Commands {

    [AutoDiscover("10.0.6435")]
    public class class_988 : ICommand {

        public short ID { get; set; } = 5461;
        public int var_2336 = 0;
        public int var_1567 = 0;

        public class_988(int param1 = 0, int param2 = 0) {
            this.var_1567 = param1;
            this.var_2336 = param2;
        }

        public void Read(IDataInput param1, ICommandLookup lookup) {
            this.var_2336 = param1.ReadInt();
            this.var_2336 = param1.Shift(this.var_2336, 20);
            this.var_1567 = param1.ReadInt();
            this.var_1567 = param1.Shift(this.var_1567, 15);
            param1.ReadShort();
        }

        public void Write(IDataOutput param1) {
            param1.WriteShort(ID);
            this.method_9(param1);
        }

        protected void method_9(IDataOutput param1) {
            param1.WriteInt(param1.Shift(this.var_2336, 12));
            param1.WriteInt(param1.Shift(this.var_1567, 17));
            param1.WriteShort(14929);
        }
    }
}
